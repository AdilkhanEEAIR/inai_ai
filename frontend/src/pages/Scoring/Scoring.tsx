      ...
      <div className={s.result__info}>
        ...
        {result.monthlyPayment > 0 && (
          <div className={s.infoRow}>
            ...
            <span className={s.infoValue} style={{ color: '#5DCAA5', fontWeight: 600 }}>
        ...
        {result.dti_tier && (
          <div className={s.infoRow}>
            <span className={s.infoLabel}>Лимит DTI:</span>
            <span className={s.infoValue} style={{ fontSize: 12 }}>{result.dti_tier}</span>
          </div>
        )}
        {result.ml_score != null && (
          <div className={s.infoRow}>
            <span className={s.infoLabel}>ML надёжность:</span>
            <span className={s.infoValue} style={{ color: riskColor }}>{result.ml_score.toFixed(1)}%</span>
          </div>
        )}
      </div>

      <div className={s.result__explain}>
        {result.reason || riskDesc}
      </div>
...
export default function ScoringPage() {
  ...
  const handleSubmit = async () => {
    ...
    const loanTermMonths = parseInt(form.loanTermMonths) || 24
    ...
    try {
      ...
      const rate = 0.08 + mockP * 0.25
      const monthlyR = rate / 12
      const n = loanTermMonths
      const mockPay = Math.round(loanAmount * (monthlyR * Math.pow(1 + monthlyR, n)) / (Math.pow(1 + monthlyR, n) - 1))
      const maxLoan = Math.round(netIncome * n * (1 - mockP))
      ...
      setResult({
        ...
        recommendedRate: Math.round((rate * 100) * 10) / 10,
        monthlyPayment: mockPay,
        top_factors: [
          ...
          { name: 'DTI лимит', contribution: -0.11 * (1 - mockP) },
        ],
        ...
        reason: mockP < 0.5 ? 'Демо-режим: все проверки пройдены.' : 'Демо-режим: высокий риск.',
      })
    } finally {
      setLoading(false)
    }
        ...
        <div className={s.content}>
          <div className={s.form_panel}>
            <div className={s.form_grid}>
              ...
              <SliderField label="Срок кредита" name="loanTermMonths" min={3} max={120} step={3} value={form.loanTermMonths} onChange={handleChange} unit="мес" required />
            </div>

            {error && <div className={s.error}>{error}</div>}

            <button className={`btn-primary ${s.submit_btn}`} onClick={handleSubmit} disabled={loading}>
              {loading
