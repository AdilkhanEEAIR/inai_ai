...
export default function LoginPage() {
  ...
  const [regData,       setRegData]       = useState({
    ...
    phone: '', birthDate: '', monthlyIncome: '', employmentYears: '',
  })
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (regData.password !== regData.confirmPassword) { setRegError(t.auth.passwordMismatch); return }
    if (regData.password.length < 6) { setRegError(t.auth.passwordTooShort); return }
    try {
      await register({
        ...
        {activeTab === 'login' && (
          <form onSubmit={handleLogin} className={s.form}>
            ...
            <button type="submit" className="btn-primary"
              ...
              {isLoading
                ? <><span className={s.spin} /><span>{t.common.enter}</span></>
        ...
        {activeTab === 'register' && (
          <form onSubmit={handleRegister} className={s.form}>
            ...
            <div className={s.formGrid}>
              <div className={s.field}>
                ...
                <input type="text" placeholder="Иванов Иван Иванович"
                  value={regData.fullName} onChange={(e) => updateRegField('fullName', e.target.value)} />
              </div>
              <div className={s.field}>
                <label>{t.auth.email} *</label>
                <input type="email" placeholder="you@example.com"
                  value={regData.email} onChange={(e) => updateRegField('email', e.target.value)} />
              </div>
              <div className={s.field}>
                <label>{t.auth.phone}</label>
                <input type="tel" placeholder="+996 700 000 000"
                  value={regData.phone} onChange={(e) => updateRegField('phone', e.target.value)} />
              </div>
              <div className={s.field}>
                <label>{t.auth.birthDate}</label>
                <input type="date" value={regData.birthDate}
                  onChange={(e) => updateRegField('birthDate', e.target.value)} />
              </div>
              <div className={s.field}>
                <label>{t.auth.monthlyIncome}</label>
                <input type="number" placeholder="150000"
                  value={regData.monthlyIncome} onChange={(e) => updateRegField('monthlyIncome', e.target.value)} />
              </div>
              <div className={s.field}>
                <label>{t.auth.employmentYears}</label>
                <input type="number" step="0.5" placeholder="3"
                  value={regData.employmentYears} onChange={(e) => updateRegField('employmentYears', e.target.value)} />
              </div>
              <div className={s.field}>
                <label>{t.auth.password} *</label>
                <input type="password" placeholder="••••••••"
                  value={regData.password} onChange={(e) => updateRegField('password', e.target.value)} />
              </div>
              <div className={s.field}>
                <label>{t.auth.confirmPassword} *</label>
                <input type="password" placeholder="••••••••"
                  value={regData.confirmPassword} onChange={(e) => updateRegField('confirmPassword', e.target.value)} />
              </div>
            </div>
            ...
            <button type="submit" className="btn-primary"
              style={{ width: '100%', justifyContent: 'center', padding: '13px' }} disabled={isLoading}>
              {isLoading
                ? <><span className={s.spin} /><span>{t.common.registering}</span></>
